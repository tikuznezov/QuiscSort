...
#define SHOW_ARRAY2        PGREEN PrintArray(&array[0], array_len); DEF_COL

#define SHOW_ARRAY1        PGREEN PrintArray(&array[0], array_len); DEF_COL
...
#define SHOW_RIGHT_SEP     printf("\nРазделение ПРАВОГО массива с %d по %d; Длина = %d\n", right_pointer, array_len - 1, array_len - right_pointer - 1); PrintArray(Raddress, array_len - right_pointer - 1);

#define SHOW_BASE          PRED printf("base = %d\n", base); DEF_COL

#define SHOW_START_INFO    printf("start: l = %zu(%d), r = %zu(%d)\n", l, array[l], r, array[r]); PrintSortingArray(array, array_len, l, r, base, iter_counter); printf("\n");

#define SHOW_SORT_STEP     PrintSortingArray(array, array_len, l, r, base, iter_counter);
